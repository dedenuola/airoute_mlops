...
def predict(req: PredictRequest):
    ...
    try:
        ...
        store = None # allow fallback below
    try:
        model = get_model()
    except Exception as e:
...
# ---------- Try Feast online features first ----------
    X = None
    if store is not None:
        ...
        try:
            ...
            # Build single-row DF and check for missing
            row = {k: (online_feats[k][0] if isinstance(online_feats[k], list) else online_feats[k])
                   for k in FEATURE_LIST}
            X_try = pd.DataFrame([row], columns=FEATURE_LIST)

            if X_try.isnull().any(axis=1).iloc[0]:
                logger.warning("[predict] Feast returned missing values; switching to S3 fallback for %s", req.site_id)
            else:
                X = X_try.astype(float)

        except Exception as e:
            logger.exception("[warn] get_online_features failed; switching to S3 fallback")

    # ---------- Fallback to S3 parquet if Feast path failed/missing ----------
    if X is None:
        X = read_latest_row_from_s3(req.site_id)
